...
type jobsDBPartitionBuffer struct {
	...
	refreshBufferedPartitionsOnEveryStore bool // if having different reader/writer DBs, we need to refresh buffered partitions on every Store
	...
	// nolint: unused // TODO: to be used in Store implementation
	flushMu sync.RWMutex // mutex to protect flush operations
...
// getBufferedPartitionsSnapshot returns a snapshot, clone of the current buffered partitions
func (b *jobsDBPartitionBuffer) getBufferedPartitionsSnapshot() *readOnlyMap[string, struct{}] {
	b.bufferedPartitionsMu.RLock()
	defer b.bufferedPartitionsMu.RUnlock()
	return b.bufferedPartitions
}
